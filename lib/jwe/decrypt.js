...
async function decrypt(opts, jwe) {
  ...
  ["encrypted_key", "iv", "ciphertext", "tag"].forEach(m => {
    let v = jwe[m];
    if (!v) {
      return;
    }
  ...
  kekCipher = params.wrappingCipher = JWA.cipher(params.header.alg);
  cekCipher = params.cipher = JWA.cipher(params.header.enc);
  ...
  if ("deriveKey" in kekCipher) {
    ...
    params = await kekCipher.unwrapKey(params);
  } else {
    throw new Error("invalid key management cipher");
  }
